...
> 本文链接：[GitHub项目徽章的添加和设置](https://github.com/maoqiqi/blog/blob/master/pages/git/git_badge.md)    
...
GitHub徽标的官方网站是 [http://shields.io/][1] ,GitHub地址是 [https://github.com/badges/shields][2] ,
...
* 持续集成状态
  ...
  持续集成的话推荐 [Travis CI][3] ,针对开源项目免费,所以你的私有项目无法享受到免费的持续构建服务,不过我们的目的貌似就是给开源项目添加徽标。
  
  同类型的产品还有 [Circle CI][4] ,不过目前跑OSX项目需要额外付费,免费版提供一个Linux项目队列,作为非付费用户在这里不多做评价,大佬们可以自己试下。
  
  其他还有诸如 [Jenkins][5] 和 [CodeShip][6] 等,大家可以在 [http://shields.io/][1] 的Build这一栏自行翻阅。
  
  Travis CI的集成工作请参考 [利用Travis CI让你在GitHub上的项目持续构建][7] 。
...
* 代码分析
  ...
  [CodeBeat][9]可以计算全局项目评分、GPA、和不同命名空间的等级来帮助您量化技术债务和发现重构机会,你唯一需要做的
  就是连接你的Github库,获得反馈就好了。
  
* 开源协议类型
  
  如果你的发布工具不提供开源协议类型的徽标的话,可以用自定义徽标的方式实现。
...
如果以上这些徽标没有满足你的需求,我们还可以定制自己的个性化徽标,[http://shields.io/][1]提供了添加自定义徽标的功能,
...
* logo
  ...
  编码然后直接插入到URL中(可以用[http://b64.io/][10]将图片转为base64编码的字符串),格式如下:
...
[7]:https://github.com/maoqiqi/blog/blob/master/pages/git/travis_ci.md
[8]:https://codecov.io/
[9]:https://codebeat.co/
[10]:http://b64.io/